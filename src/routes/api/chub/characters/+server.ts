...
export const GET: RequestHandler = async (event) => {
	...
	const minTokens = Number.isFinite(minTokensRaw) && minTokensRaw >= 0 ? Math.min(100_000, Math.floor(minTokensRaw)) : 50;
	...
	try {
		...
		logger.warn('chub characters search error', { err: err instanceof Error ? err.message : String(err) });
